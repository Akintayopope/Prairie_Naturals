class CheckoutController < ApplicationController
  ...
  def create
    unless Stripe.api_key.present?
      redirect_to cart_path,
                  alert: "Payments unavailable (missing Stripe key)." and return
    end
    ...
    if params[:order_id].present?
      ...
      if order.status == "paid"
        return(
          redirect_to order_path(order), notice: "This order is already paid."
        )
      end
    ...
    if @order.province.blank?
      return prepare_failed_new(cart_items, "Please choose a province.")
    end

    province_record = Province.find_by(name: @order.province)
    unless province_record
      return prepare_failed_new(cart_items, "Please choose a valid province.")
    end
    ...
    prepare_failed_new(
      ...
      "Validation error: #{e.record.errors.full_messages.to_sentence}"
    )
  ...
  def success
    if params[:session_id].blank?
      ...
      @order ||=
        current_user.orders.where(status: :paid).order(created_at: :desc).first
      redirect_to(
    ...
    order =
      Order.find_by(id: s.metadata["order_id"]) ||
        Order.find_by(stripe_session_id: s.id)

    unless order
      redirect_to orders_path, alert: "Order not found for this payment." and
        return
    end
  ...
  def preview_receipt
    cart_items = current_user.cart_items.includes(:product)
    if cart_items.none?
      return redirect_to(new_checkout_path, alert: "Your cart is empty.")
    end
    ...
    ship_to = [shipping_address, city, province_name, postal_code].reject(
      &:blank?
    ).join(", ")
    ...
    pdf.move_down 10
    pdf.text "Note: This is a preview/quote. Final receipt is issued after successful payment.",
             size: 9,
             style: :italic
