  ...
  ActiveAdmin.register User, as: "Customer" do
        ...
        super do |success, failure|
          success.html do
            if @send_reset_after_create &&
                 resource.respond_to?(:send_reset_password_instructions)
              resource.send_reset_password_instructions
              flash[
                :notice
              ] = "Customer created. Reset password email sent to #{resource.email}."
            else
    ...
    action_item :reset_password, only: :show do
      if resource.respond_to?(:send_reset_password_instructions)
        path =
          begin
            url_for([:reset_password, :admin, resource])
          rescue StandardError
            "#{resource_path(resource)}/reset_password"
          end
    ...
    member_action :reset_password, method: :post do
      if resource.respond_to?(:send_reset_password_instructions)
        ...
        redirect_to resource_path(resource),
                    notice:
                      "Reset password instructions sent to #{resource.email}."
      else
        redirect_to resource_path(resource),
                    alert: "Reset password is not enabled for this model."
      end
    end
    ...
    index title: "Customers" do
      ...
      column("Orders") do |u|
        number_with_delimiter(u.attributes["orders_count_calc"].to_i)
      end
      column("Total Tax") do |u|
        number_to_currency(u.attributes["lifetime_tax"].to_d)
      end
      column("Grand Total") do |u|
        number_to_currency(u.attributes["lifetime_total"].to_d)
      end
      actions
    end
    ...
    show title: proc { |u| "Customer ##{u.id} — #{u.email}" } do
      attributes_table do
        ...
        row("Lifetime Total") do |u|
          number_to_currency(u.orders.sum(:total) || 0)
        end
      end
    ...
    form do |f|
      ...
      f.inputs "Customer" do
        ...
        if User.column_names.include?("province_id") &&
             ActiveRecord::Base.connection.data_source_exists?("provinces")
          f.input :province,
                  as: :select,
                  collection: Province.order(:name).pluck(:name, :id),
                  include_blank: true
        end
