...
class WalmartSerpapiImporter
  ...
  CATEGORY_MAP = [
    [
      /ashwagandha|turmeric|ginseng|supplement|vitamin|capsule|tablet/i,
      "Vitamins"
    ],
  ...
  def import_keyword(keyword, limit: 50, category_name: nil, enrich_n: 0)
    ...
    enrich_n = enrich_n.to_i.clamp(0, 10)
    ...
    results
      ...
      .each_with_index do |r, idx|
        ...
        desc_html =
          pr["short_description_html"] || r["description"] || r["snippet"] || ""
        ...
        image_url =
          pick_image_url(pr) || r["thumbnail"] || r["image"] ||
        ...
        product.review_count = review_count if review_count &&
          product.respond_to?(:review_count=)

        product.save!

        if image_url.present? && product.respond_to?(:images) &&
        ...
        imported += 1
        Rails.logger.info(
          "Imported (#{idx + 1}/#{limit}): #{product.name} → #{category.name}"
        )
      rescue => e
        Rails.logger.warn(
          "Import failed for #{product_id.inspect}: #{e.class} – #{e.message}"
        )
        next
      end
  ...
  def resolve_category(keyword:, serpapi_item:, title:)
    ...
    (match && match.last) || "Vitamins"
  end

  def extract_price_cents(r, pr)
    raw =
      pr.dig("price_map", "price") || pr.dig("price", "raw") ||
        r.dig("primary_offer", "offer_price") ||
        r.dig("primary_offer", "min_price") || r["price"] ||
        r.dig("price", "raw")
  ...
  def attach_first_image(product, url, product_id)
    ...
    Rails.logger.warn(
      "Image attach failed for #{product_id}: #{e.class} – #{e.message}"
    )
  end
end
